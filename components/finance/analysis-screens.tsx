...
export function SpendAnalysisScreen() {
  ...
  return <div className="grid gap-7">
    ...
    <div data-finwise-motion="analysis-stat-collection" className="finwise-stagger grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><Stat label="Total spending" value={formatMoney(total, currency)} /><Stat label="Average spending" value={formatMoney(expenses.length ? total / expenses.length : 0, currency)} /><Stat label="Largest expense" value={largest ? formatMoney(largest.amount, currency) : "—"} /><Stat label="Transactions" value={String(expenses.length)} /><Stat label="Top category" value={categoryData[0]?.name ?? "—"} /><Stat label="Small purchases" value={String(expenses.filter((item) => monetary(item.amount) < (settings?.small_purchase_threshold ?? 100)).length)} /></div>
