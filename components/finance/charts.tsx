...
export function OnlineCashBalanceChart({ data, currency = DEFAULT_WORKSPACE_CURRENCY }: { data: { label: string; online: number; cash: number }[]; currency?: string }) {
  if (!data.some((row) => row.online || row.cash)) return <ChartEmpty text="Add a Bank, Other, or Cash Wallet account to see your everyday money balance over time." />;
  return <ResponsiveContainer width="100%" height={250}><AreaChart data={data} margin={{ top: 12, right: 4, left: -25, bottom: 0 }}><defs><linearGradient id="onlineBalanceFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#C6A15A" stopOpacity={.2} /><stop offset="100%" stopColor="#C6A15A" stopOpacity={0} /></linearGradient></defs><XAxis dataKey="label" stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={11} /><YAxis stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={11} tickFormatter={(value) => formatMoney(value, currency, true)} /><Tooltip contentStyle={tooltipStyle} formatter={(value: number, name) => [formatMoney(value, currency), name === "online" ? "UPI / online" : "Cash Wallet"]} /><ReferenceLine y={0} stroke="var(--line)" /><Area type="monotone" dataKey="online" name="UPI / online" stroke="#C6A15A" fill="url(#onlineBalanceFill)" strokeWidth={2.4} activeDot={{ r: 4, fill: "#C6A15A" }} /><Area type="monotone" dataKey="cash" name="Cash Wallet" stroke="#98ABB8" fill="transparent" strokeWidth={2.1} activeDot={{ r: 4, fill: "#98ABB8" }} /></AreaChart></ResponsiveContainer>;
}
...
export function NetWorthChart({ data, currency = DEFAULT_WORKSPACE_CURRENCY }: { data: { label: string; netWorth: number }[]; currency?: string }) {
  if (!data.some((row) => row.netWorth)) return <ChartEmpty text="Net-worth history will appear once your accounts and ledger contain balances." />;
  return <ResponsiveContainer width="100%" height={270}><AreaChart data={data} margin={{ top: 12, right: 4, left: -25, bottom: 0 }}><defs><linearGradient id="netWorthFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#73B68B" stopOpacity={.24} /><stop offset="100%" stopColor="#73B68B" stopOpacity={0} /></linearGradient></defs><XAxis dataKey="label" stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={11} /><YAxis stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={11} tickFormatter={(value) => formatMoney(value, currency, true)} /><Tooltip contentStyle={tooltipStyle} formatter={(value: number) => [formatMoney(value, currency), "Net worth"]} /><ReferenceLine y={0} stroke="var(--line)" /><Area type="monotone" dataKey="netWorth" name="Net worth" stroke="#73B68B" fill="url(#netWorthFill)" strokeWidth={2.4} activeDot={{ r: 4, fill: "#73B68B" }} /></AreaChart></ResponsiveContainer>;
}

export function RhythmChart({ data, currency = DEFAULT_WORKSPACE_CURRENCY }: { data: { label: string; value: number }[]; currency?: string }) {
  if (!data.some((row) => row.value)) return <ChartEmpty text="Your usual spending rhythm will appear after a few recorded expenses." />;
  return <ResponsiveContainer width="100%" height={220}><BarChart data={data} margin={{ top: 8, right: 4, left: -22, bottom: 0 }}><XAxis dataKey="label" stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={10} /><YAxis stroke="var(--muted)" tickLine={false} axisLine={false} fontSize={11} tickFormatter={(value) => formatMoney(value, currency, true)} /><Tooltip contentStyle={tooltipStyle} formatter={(value: number) => [formatMoney(value, currency), "Average spending"]} /><Bar dataKey="value" fill="#C6A15A" radius={[7, 7, 0, 0]} /></BarChart></ResponsiveContainer>;
}
