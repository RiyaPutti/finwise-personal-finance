...
export const budgetSchema = z.object({ category_id: z.string().uuid(), amount: money, starts_on: z.string().date(), ends_on: z.string().date() }).refine((value) => value.ends_on >= value.starts_on, { message: "End date must follow the start date.", path: ["ends_on"] });
